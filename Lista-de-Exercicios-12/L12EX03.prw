...
Static Function ImpDados(cAlias)
    ...
    while (cAlias)->(!EOF())
        ...
        if cCodCurso <> Alltrim((cAlias)->(ZZC_COD))
                ...
                oExcel:AddColumn(cPasta, cTable, PADR('Código',10)    , CENTRO    , GERAL)
                oExcel:AddColumn(cPasta, cTable, PADR('Nome Aluno',50), ESQUERDA  , GERAL)
                oExcel:AddColumn(cPasta, cTable, PADR('Idade',8)      , CENTRO    , NUMERICO)
        endif
