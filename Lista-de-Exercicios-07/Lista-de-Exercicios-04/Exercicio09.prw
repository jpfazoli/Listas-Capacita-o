...
User Function L7Ex09()
    local oCombo    := NIL
    local oDlg      := NIL
    local cCond     := 0
    local aGenero   := {'1=Homem','2=Mulher'}
    local lValido   := .T.
    local nCont     := 0
    local nAltura   := 0
    local nPeso     := 0
    ...
    while lValido
        ...
        @ 105,040 MSCOMBOBOX oCombo VAR cCond ITEMS aGenero SIZE 60,15 OF oDlg PIXEL 
        ...
        if nCont > 0 //Valida se é a primeira execução da tela
