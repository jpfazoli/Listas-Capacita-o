...
Static Function GeraRelatorio()
    ...
    local oRelatorio := TReport():NEW('L10EX03', 'Relatório do Pedido de Compra',, {|oRelatorio| ImprimeDados(oRelatorio,cAlias)}, 'Relatório para Impressão do Pedido de Compra', .F.)
    local oSection1   := TRSection():NEW(oRelatorio, 'Seção do Pedido')
    local oSection2   := TRSection():NEW(oRelatorio, 'Seção dos Itens')
    ...
    TRCELL():New(oSection2, 'C7_PRECO'   , 'SC7', 'Preço'            ,'@!'             , 10   ,,, 'LEFT'  ,.F., 'LEFT'  ,,, .T.,,, .T.)
    TRCELL():New(oSection2, 'C7_TOTAL'   , 'SC7', 'Total'            ,"@!"             , 10   ,,, 'LEFT'  ,.F., 'LEFT'  ,,, .T.,,, .T.)
...
Static Function ImprimeDados(oRelatorio,cAlias)
    ...
    while (cAlias)->(!EOF())
        ...
        cTotal :=  'R$' + Alltrim(STR((cAlias)->(C7_TOTAL),,2))
        ...
        oSection2:Cell('C7_PRECO'):SetValue(cPreco)
        oSection2:Cell('C7_TOTAL'):SetValue(cTotal)
